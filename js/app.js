...
(function () {
  ...
  const state = {
    ...
    filter: {
      ...
      leasing:       "",           // "" = alle, "ja" = nur Leasing, "nein" = ohne Leasing
      ebike:         ""            // "" = alle, "ja" = nur E-Bikes, "nein" = nur normale Bikes
    },
    settings: {
  ...
  function updateFilterLabel() {
    ...
    if (state.filter.leasing === "ja")       extra.push("Leasing");
    if (state.filter.leasing === "nein")     extra.push("Kein Leasing");
    if (state.filter.ebike === "ja")         extra.push("E-Bike");
    if (state.filter.ebike === "nein")       extra.push("Kein E-Bike");
  ...
  function initFilterDropdowns() {
    ...
    // Leasing-Filter (client-side)
    const fl = document.getElementById("filterLeasing");
    if (fl) fl.addEventListener("change", (e) => {
      state.filter.leasing = e.target.value;   // "", "ja", "nein"
      updateFilterLabel();
      reRenderCurrentTab();
    });
    // E-Bike-Filter (client-side)
    const fe = document.getElementById("filterEbike");
    if (fe) fe.addEventListener("change", (e) => {
      state.filter.ebike = e.target.value;     // "", "ja", "nein"
      updateFilterLabel();
      reRenderCurrentTab();
    });
    ...
    document.getElementById("resetFilters").addEventListener("click", () => {
      ...
      state.filter.leasing = "";
      state.filter.ebike = "";
      ["filterMarke","filterKategorie","filterPickstatus","filterLagerplatz",
       "filterMontiert","filterVerpackt","filterLeasing","filterEbike"].forEach(id => {
  ...
  function buildOffeneRow(r) {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(r.itemid || r.artikel || "")}${r.is_ebike ? ' <span class="badge badge-ebike" title="E-Bike">⚡</span>' : ''}</td>
      ...
      <td>${zahlungsartCell(r)}</td>
      <td class="mono small">${esc(r.licenseplate || "")}</td>
      <td class="mono small">${esc(r.framenumber || "")}</td>
  ...
  /* Zahlungsart-Zelle mit Leasing-Highlight */
  function zahlungsartCell(r) {
    const za = (r.zahlungsart || "").trim();
    if (!za) return "";
    if (r.ist_leasing) {
      return `<span class="badge badge-leasing" title="Leasing: ${esc(za)}">${esc(za)}</span>`;
    }
    return `<span class="muted">${esc(za)}</span>`;
  }
  ...
  function buildFaktRow(r) {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(r.salesid || "")}</td>
      <td>${fmtDate(r.datum)}</td>
      <td>${esc(r.itemid || "")}${r.is_ebike ? ' <span class="badge badge-ebike" title="E-Bike">⚡</span>' : ''}</td>
      <td>${esc(r.marke || "")}</td>
      <td>${esc(r.modell || "")}</td>
      <td>${esc(r.produktgruppe || "")}</td>
      <td>${esc(r.fahrradtyp || "")}</td>
  ...
  function filterRows(rows, query) {
    ...
    // 8. Leasing-Filter
    if (state.filter.leasing === "ja") {
      out = out.filter(r => r.ist_leasing === true);
    } else if (state.filter.leasing === "nein") {
      out = out.filter(r => r.ist_leasing !== true);
    }

    // 9. E-Bike-Filter
    if (state.filter.ebike === "ja") {
      out = out.filter(r => r.is_ebike === true);
    } else if (state.filter.ebike === "nein") {
      out = out.filter(r => r.is_ebike !== true);
    }
